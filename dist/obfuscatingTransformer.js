...
function obfuscatingTransformer(_a) {
    ...
    var obfuscatorOptions = __assign({}, _obfuscatorOptions, { sourceMap: true, sourceMapMode: "separate" });
    return {
        transform: function (props) {
            ...
            if (resultCanBeObfuscated && filter(props.filename, props.src)) {
                ...
                var newCode = obfuscateCode_1.obfuscateCodePreservingSourceMap(code, map, props.filename, props.src, obfuscatorOptions);
                if (otherOptions.emitObfuscatedFiles) {
                    ...
                    fs.writeFileSync(path.join(emitDir, extendFileExtension_1.extendFileExtension(path.basename(props.filename), "original")), code);
                    fs.writeFileSync(path.join(emitDir, filename), newCode.code);
                }
                return getMetroTransformer_1.maybeTransformMetroResult(result, newCode);
            }
            return result;
        },
